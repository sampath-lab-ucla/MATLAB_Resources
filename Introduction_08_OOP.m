...
% 
% The short answer is: no. The long answer is: its, at worst, the same amount 
% of work but typically, especially if you have multiple members of the same type 
% (multiple instances), much less work. The key is in the properties.
% 
...
% Back to quails...
% **Run this section by double clicking your cursor here and entering the keyboard 
% command: Ctrl (or Cmd) + Enter
% 
% So let's start at a common entry point for our California Quail and, say, 
% a scaled quail. These both belong to the class, Aves. More specifically, a litte 
...
% **Run this section by placing your cursor here and entering the keyboard command: 
% Ctrl (or Cmd) + Enter
% 
...
% and so on. So we can, by creating a subclass of Aves called Quails, add on to 
% our list of properties, and we can set some values for our blank properties. 
% To do this, we need to create a special method call a _constructor_. Another 
% feature of OOP is one where we can also override, modify or add onto superclass 
% methods.
%%
% 
...
%
%% 
% We have given Quails a headMarkings property, but left if blank, defined that 
% all quails canFly = true but preferFlight = false. In order to incorporate these 
% changes into our quail, we needed to add onto the fly method. The first line 
...
QuailsMember.fly()
%% 
% As you can see, Quails can fly but prefer to walk. But if we asked for their 
% call:

QuailsMember.call()
%% 
% We don't have a generic quails call, mostly because quail species have very 
% different calls from each other. Two examples of this would be the California 
% Quail and Scaled Quail. They have descriptions that can be coded into new objects 
% like so:
%%
% 
...
% birds to sing (press the button and make sure your speakers are on):
...
% classes away. Because the Quails class adds on to the Aves class' fly method 
% and Quails is the superclass of our species levels classes, we use the fly method 
